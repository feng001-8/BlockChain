...
// 5. 定义一个区块链
func NewBlockChain(address string, reward float64) *BlockChain {
	//return &BlockChain{
	...
	db.Update(func(tx *bolt.Tx) error {
		...
		if bucket == nil {
			...
			genesisBlock := GenesisBlock(address, reward)
...
// 定义一个创世块
func GenesisBlock(address string, reward float64) *Block {
	coinbase := NewCoinbaseTX(address, "交易的第一块", reward)
